...
main (int argc, char *argv[])
{
	...
	while ((c = getopt (argc, argv, "tha:p:s:r:")) != -1)
	switch (c)
	{
	case 't':
		usetestsource = 1;
	...
	case 'h':
		...
		fprintf(stderr,"       -r [bitrate] opus bitrate: 6000 (default),8000,12000,32000,64000,128000 \n\n");
		return 1;
	case 's':
	    audiosource = optarg;
	    break;
	...
	g_object_set (G_OBJECT ( opusencoder ), "bitrate", bitrate, NULL); 	// 6000 - 128000
